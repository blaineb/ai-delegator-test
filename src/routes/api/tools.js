...
export const greenTechChannel = [
  ...
  {
    "author": "Lisa",
    "message": "Update: The client is open to increasing their budget to $600,000."
  },
  {
    "author": "Carmen",
    "message": "That's great news! We can adjust our negotiation strategy accordingly."
  },
  {
    "author": "Arcadio",
...
export async function summarizeChannel(channel) {
  let channelData;
  try {
    channelData = JSON.parse(channel);
  } catch (error) {
    console.error("Failed to parse channel data. Please ensure it's a valid JSON string.");
    throw error;
  }
  messages.push({
    role: "user",
...
export async function getSalesforceRecords({ name, ownerName, accountName, dealSize, stage } = {}) {
  return salesforceDB.filter(record => {
    console.log("Name ask:" + name);
    console.log("Record name" + record.name);
    return (!name || record.name.toLowerCase() === name.toLowerCase()) &&
      (!ownerName || record.ownerName.toLowerCase() === ownerName.toLowerCase()) &&
      (!accountName || record.name.toLowerCase() === accountName.toLowerCase()) &&
...
export async function generalKnowledge(query) {
  ...
  const checkRunStatus = async (threadId, runId) => {
    ...
    if (runStatus.status === "completed") {
      ...
      console.log("The response:" + JSON.stringify(threadUpdate.data[0].content, null, 2));
    }
    else {
...
export async function updateDeal(name) {
  ...
  let message = await openai.beta.threads.messages.create(thread.id, {
    role: "user",
    content: "Look through the channel's data and find any updates to the deal that need to happen to the data." +
      `Here is the existing info we have for the deal: ${JSON.stringify(dealData)}` +
      `And here is the channel information: ${JSON.stringify(eval(channelData))}`
  });
